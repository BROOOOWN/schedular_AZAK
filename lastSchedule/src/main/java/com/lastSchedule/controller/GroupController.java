...
//    }
    ...
    @GetMapping(value = {"/list"})
    public String groupManage(GroupSearchDto searchDto, Long groupId, @PathVariable(value = "page") Optional<Integer> page, Model model, Principal principal){
        Pageable pageable = PageRequest.of(page.isPresent() ? page.get() : 0, 5);
        searchDto.setGroupId(groupId);
        Page<Group> groups = groupService.getAdminGroupPage(searchDto, groupId, pageable);
        //현재 로그인한 사람의 이름=아이디
        String email = principal.getName();
        //로그인한 회원의 멤버 아이디 조회
        Long loggedInMemberId  = memberService.getMemberIdByEmail(email);
        //로그인한 회원의 그룹 조회
        List<Group> loggedgroups = groupService.getGroupsByLoggedInMemberId(loggedInMemberId);

        Optional<String> groupName = loggedgroups.stream()
                .filter(group -> group.getId().equals(groupId))
                .map(Group::getGroupName).findFirst();

        for (Group group : loggedgroups) {
            if(StringUtils.isEmpty(group.getSpecialNote())){
                group.setSpecialNote("무");
            }else{
                group.setSpecialNote("유");
